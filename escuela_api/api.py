...
def create_asistencia():
    ...
    return '<h1>REGISTRO COMPLETO POR Erick Gallegos</h1>'

@app.route('/update_asistencia/<cedula>', methods=['PUT'])
def update_asistencia(cedula):
    with open('datos\listado_de_asistencia.csv') as file:
    ...
    with open('datos\listado_de_asistencia.csv', 'w', newline='') as file:
        writer = csv.writer(file, delimiter=',')
        writer.writerow(['cedula', 'materia', 'fecha_anio', 'fecha_mes', 'fecha_dia'])
        for asistencia in lista:
            writer.writerow([asistencia['cedula'], asistencia['materia'], asistencia['fecha_anio'], asistencia['fecha_mes'], asistencia['fecha_dia']])
    return '<h1>REGISTRO MODIFICADO POR Erick Gallegos</h1>'
